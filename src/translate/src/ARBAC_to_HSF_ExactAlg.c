...
static void relation_with_admin_shorthand(FILE *outputFile, int user_index, int admin_role_index)
{
    ...
    fprintf(outputFile, ", _%s=1", tracked_user_and_role(user_index, admin_role_index));
}
...
configuration_user(FILE *outputFile, int user_index)
{
    ...
    for (i = 0; i < NUM_USER_TO_TRACK; i++)
    {
        ...
        for (j = 0; j < role_array_size; j++)
        {
            ...
            {
                fprintf(outputFile, ", _%s_1", tracked_user_and_role(i, j));
            }
            else
            {
                fprintf(outputFile, ", _%s", tracked_user_and_role(i, j));
        ...
        for (j = 0; j < role_array_size; j++)
        {
            ...
            if (belong_to(user_config_array[user_index].array, user_config_array[user_index].array_size, j))
            {
                fprintf(outputFile, ", _%s_1=1", tracked_user_and_role(i, j));
            }
        }
        fprintf(outputFile, ".\n");
    }
    ...
    for (i = 0; i < NUM_USER_TO_TRACK; i++)
    {
        for (h = 0; h < NUM_USER_TO_TRACK; h++)
        {
            ...
            for (k = 0; k < role_array_size; k++)
            {
                ...
                {
                    fprintf(outputFile, ", _%s=1", tracked_user_and_role(i, k));
                }
                else if (belong_to(ca_array[ca_rule].negative_role_array, ca_array[ca_rule].negative_role_array_size, k))
                {
                    fprintf(outputFile, ", _%s=0", tracked_user_and_role(i, k));
                }
                else if (k == ca_array[ca_rule].target_role_index)
                {
                    fprintf(outputFile, ", _%s_1=1", tracked_user_and_role(i, k));
                }
            }
            fprintf(outputFile, ".\n");
        }
...
simulate_can_revoke_rule(FILE *outputFile, int cr_rule)
{
    ...
    for (i = 0; i < NUM_USER_TO_TRACK; i++)
    {
        for (h = 0; h < NUM_USER_TO_TRACK; h++)
        {
            ...
            relation_shorthand(outputFile, i);
            fprintf(outputFile, ", _b%d=1", i);
            fprintf(outputFile, ", _%s=1", tracked_user_and_role(i, cr_array[cr_rule].target_role_index));
            fprintf(outputFile, ", _%s_1=0", tracked_user_and_role(i, cr_array[cr_rule].target_role_index));
            fprintf(outputFile, ".\n");
        }
    }
}
