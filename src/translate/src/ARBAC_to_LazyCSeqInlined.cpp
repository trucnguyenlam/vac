...
generate_updates(FILE *outputFile, int thread_id) {
    ...
    #else
    #ifdef NO_GLOBALS
    fprintf(outputFile, "    /*--- NO GLOBALS VERSION USED ---*/\\\n");
    #else
...
simulate_can_assign(FILE *outputFile, int thread_id, int ca_index, int label_index) {
    fprintf(outputFile, "_ ## ROUND ## _tThread_%d_%d:\\\n", thread_id, label_index);
...
generate_thread(FILE *outputFile, int thread_id) {
    ...
    fprintf(outputFile, "#define Thread_%d(ROUND) \\\n", thread_id);    
...
static void
generate_thread_step(FILE *outputFile, int thread_id, int steps, int round) {
    ...
    fprintf(outputFile, "    Thread_%d(%d);\n", thread_id, round);
}
...
generate_round(FILE *outputFile, int round, int steps) {
    ...
    for (int i = 0; i < threads_count; ++i) {
        generate_thread_step(outputFile, i, steps, round);
    }
    fprintf(outputFile, "\n");
}
...
void
transform_2_lazycseq_inlined(char *inputFile, FILE *outputFile, int rounds, int steps, int wanted_threads_count) {
