...
def generatePolicy(args):
    ...
    prefix = args.path + "/" + os.path.basename(args.input) if args.path != "" else os.path.basename(args.input)
    ...
    # TODO(me): hierarchy in rule
    # For rules
    rulestr = "RULES\n"
    for ca_rule in policy.ca_rules:
        rulestr += ca_rule.toVACRule() + "\n"
    for cr_rule in policy.cr_rules:
        rulestr += cr_rule.toVACRule() + "\n"
    rulestr += ";\n\n"
    ...
    for index, q in enumerate(policy.queries):
        ...
        for f, r in enumerate(q.goal):
            ...
            saveFile(prefix + "%su_%snu_query%s_%s.txt" % (args.nuser, args.nnewuser, index, f), ret)
...
def main():
    ...
    parser.add_argument('-p', '--output', metavar='X', help='output path', type=str, dest='path', default="")
    parser.add_argument('-n', '--users', metavar='X', help='generate X normal users', type=int, dest='nuser', default=2)
    parser.add_argument('-u', '--new-users', metavar='X', help='generate X new users', type=int, dest='nnewuser', default=0)
