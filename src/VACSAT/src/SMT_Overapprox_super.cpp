...
//                                                 BIT_VECTOR);
        ...
        void init_new_frame(const Expr& _target_expr, int blocks_to_do) {
            deep = deep - 1;
//            target_rules.insert(_target_rule.begin(), _target_rule.end());
            target_expr = _target_expr;
            blocks_to_do = blocks_to_do;
...
//        }
    ...
    void generate_update_state(int block_idx) {
...
//            log->critical("update {} of depth {}", round_idx, state.deep);
    ...
    void generate_round(int block_idx) {
        int label_idx = 0;
        ...
        generate_update_state(block_idx);
