...
class ExtendedOverapproxTransformer {
    ...
    static int get_pc_size(const std::shared_ptr<arbac_policy>& policy, const std::set<Atomp>& tracked_atoms) {
        int count = 1;
    ...
    public:
        ...
        overapprox_status(SMTFactory<TExpr, TVar>* _solver,
                          ...
                          std::tuple<const std::vector<bool>, // _tracked_roles,
                                     ...
                                     const int /*_pc_size*/> tracked_and_pc) :
                solver(_solver),
                policy(_policy),
...
//            }
        ...
        inline void emit_assumption(const TExpr& value) {
            //TODO: maybe put a XOR instead of OR
            TExpr guarded_ass = solver->createXorExpr(solver->createNotExpr(state.guard.get_solver_var()), value);
            solver->assertLater(guarded_ass);
        }
...
//    int pc_size;
    ...
    static std::tuple<std::vector<bool>, int, int> get_tracked_pc_size(const std::shared_ptr<arbac_policy>& policy, const Expr& phi_t) {
        bool fixpoint = false;
...
//                        print_collection(rule->prec->literals());
        ...
        int pc_size = get_pc_size(policy, to_track);
        return std::tuple<std::vector<bool>, int, int>(tracked_roles, tracked_roles_count, pc_size);
    }
...
//                int exclude = !excluded_is_ca ? exclude_idx : -1;
    ...
    ExtendedOverapproxTransformer(const std::shared_ptr<SMTFactory<TVar, TExpr>> _solver,
        ...
        state(_solver.get(), _policy, Config::overapprox_depth, get_tracked_pc_size(_policy, _to_check)),
