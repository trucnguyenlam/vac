...
//            log->warn("Popping {}", *target_expr);
    ...
    struct global_status {
        SMTFactory<TVar, TExpr>* solver;
        std::shared_ptr<arbac_policy> policy;
        const std::vector<std::shared_ptr<overapprox_status>> statuses;
        std::shared_ptr<overapprox_status> target_status;

        global_status(int count, SMTFactory<TExpr, TVar>* _solver, std::shared_ptr<arbac_policy> _policy, int _deep) :
                solver(_solver),
                policy(_policy),
                statuses((ulong) count),
                target_status(nullptr) {
            for (int i = 0; i < count; ++i) {
                statuses[i] = std::shared_ptr<overapprox_status>(new overapprox_status(solver, policy, _deep));
            }
...
//        solver->deep_clean();
    ...
    template <typename TVar, typename TExpr>
    bool overapprox_multi(const std::shared_ptr<SMTFactory<TVar, TExpr>>& solver,
    ...
    template bool overapprox_multi<term_t, term_t>(const std::shared_ptr<SMTFactory<term_t, term_t>>& solver,
                                             const std::shared_ptr<arbac_policy>& policy,
                                             const Expr& to_check,
                                             const std::set<rulep>& to_check_source);
    template bool overapprox_multi<expr, expr>(const std::shared_ptr<SMTFactory<expr, expr>>& solver,
                                         const std::shared_ptr<arbac_policy>& policy,
                                         const Expr& to_check,
                                         const std::set<rulep>& to_check_source);
